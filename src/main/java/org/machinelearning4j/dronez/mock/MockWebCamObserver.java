...
public class MockWebCamObserver extends AbstractWebCamObserver {
	...
	public void stateUpdatedAfterAction(DroneState droneState, DroneAction droneAction) {
		...
		if (droneLocation != null) {
			image.getGraphics().fillRect((int) droneLocation.getPointOnCamera().getX() - 5,
					(int) droneLocation.getPointOnCamera().getY() - 5, 10, 10);
		}
		...
		if (targetTrajectory != null) {
			image.getGraphics().drawLine(
					getX(targetTrajectory.getTarget(targetTrajectoryIteration).getLeftRightPositionVelocity()
							.getPosition()),
					0,
					getX(targetTrajectory.getTarget(targetTrajectoryIteration).getLeftRightPositionVelocity()
							.getPosition()), 480);
			image.getGraphics().drawLine(
					0,
					getY(targetTrajectory.getTarget(targetTrajectoryIteration).getUpDownPositionVelocity()
							.getPosition()),
					640,
					getY(targetTrajectory.getTarget(targetTrajectoryIteration).getUpDownPositionVelocity()
							.getPosition()));
	...
	public Position3D getDroneLocation() {
		Point point = new Point((int) ((droneState.getLeftRightPositionVelocity().getPosition() + 2.5) * 640 / 5),
				(int) ((2.5 - droneState.getUpDownPositionVelocity().getPosition()) * 480 / 5));
		return new Position3D(droneState.getLeftRightPositionVelocity().getPosition(), droneState
				.getUpDownPositionVelocity().getPosition(), droneState.getForwardBackPositionVelocity().getPosition(),
				point);
	}

	public Velocity3D getDroneVelocity() {
		return new Velocity3D(droneState.getLeftRightPositionVelocity().getVelocity(), droneState
				.getUpDownPositionVelocity().getVelocity(), droneState.getForwardBackPositionVelocity().getVelocity());
	}
