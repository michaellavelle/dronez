...
public class UpAndDownTrajectory implements Trajectory<DroneState> {
	...
	public DroneState getTarget(long iteration) {
		PositionVelocity leftRightTarget =  new PositionVelocity( initialState.getLeftRightPositionVelocity().getPosition(),0);		
		PositionVelocity upDownTarget =  new PositionVelocity( initialState.getUpDownPositionVelocity().getPosition() + 60 - 60 * Math.cos(2 * Math.PI * iteration /60),0);		
