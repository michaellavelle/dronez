...
public class DiscreteStateMdpInnerPolicyCommandFactoryImpl extends AbstractInnerOuterRegionPolicyCommandFactory {
	...
	public void updatePolicies() {
		learnedLeftRightMdp.updateProbabilitiesAndOptionallyValueFunctionAndPolicy(
				this.probabilities4D.getLeftRightProbabilitiesBuilder(), true);
		learnedUpDownMdp.updateProbabilitiesAndOptionallyValueFunctionAndPolicy(
				this.probabilities4D.getUpDownProbabilitiesBuilder(), true);
		learnedForwardBackMdp.updateProbabilitiesAndOptionallyValueFunctionAndPolicy(
				this.probabilities4D.getForwardBackProbabilitiesBuilder(), true);
	...
	private <A extends NumericAction> DiscreteIndexedStateMarkovDecisionProcessAdapter<TargetRelativePositionWithVelocity, A> createDiscreteIndexedStateSingleDimensionMarkovDecisionProcess(
		...
		RewardFunction<TargetRelativePositionWithVelocity> rewardFunction = new MinimiseTargetRelativePositionRewardFunction();

		Policy<TargetRelativePositionWithVelocity, A> initialPolicy = new RandomPolicy<TargetRelativePositionWithVelocity, A>(
				actions);
		// = new SimpleLeftRightPolicy2();

		return new DiscreteIndexedStateMarkovDecisionProcessAdapter<TargetRelativePositionWithVelocity, A>(
				stateIndexMapper, indexMapper, rewardFunction, new ZeroValueFunction<Integer>(), probabilitiesBuilder,
				initialPolicy);
	}

	protected <A extends NumericAction> Policy<TargetRelativePositionWithVelocityAndRecentActions<A>, A> createSingleDimensionDistanceToTargetPolicy(
			final DiscreteStateMarkovDecisionProcess<TargetRelativePositionWithVelocity, A> mdp, A[] actions) {
		...
		return new Policy<TargetRelativePositionWithVelocityAndRecentActions<A>, A>() {
			...
			@Override
			public A getAction(TargetRelativePositionWithVelocityAndRecentActions<A> arg0) {
