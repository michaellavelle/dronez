...
public class WebCamObserver extends AbstractWebCamObserver implements StateActionController<DroneState,DroneAction>  {
	...
	public void startObserving() {
		...
		LabelAssigner<SerializableBufferedImageAdapter, Position3D> motionCOGLabelAssigner =
			//new MotionCOGPointLabelAssigner(webcamImageExtractor.getFrameWidth(), webcamImageExtractor.getFrameHeight(),
				//	stickyCog); 

				new DronePositionDetectionPointLabelAssigner(webcamImageExtractor.getFrameWidth(), webcamImageExtractor.getFrameHeight(),
			stickyCog);
		...
		if (directoryPath != null)
		{
			...
			//new DirectoryLabeledBufferedImageWriter<Long,PointInSpace>(directoryPath,new FilenameFromTimestampAndPointGenerator("jpg")));
