...
public class TargetDisplayer
		implements
		FrameDecorator<LabeledData<SerializableBufferedImageAdapter, Position3D>, Long> {
	...
	public LabeledData<SerializableBufferedImageAdapter, Position3D> decorateFrame(
			LabeledData<SerializableBufferedImageAdapter, Position3D> arg0,
			Long arg1) {
		if (arg0 != null && arg0.getData() != null
				&& webCamObserver.getTargetTrajectory() != null) {
			BufferedImage image = arg0.getData().getImage();
			int iteration = webCamObserver.getIteration();
			Trajectory<DroneState> targetTrajectory = webCamObserver
					.getTargetTrajectory();
			image.getGraphics()
					.drawLine(
							(int) (targetTrajectory.getTarget(iteration)
									.getLeftRightPositionVelocity()
									.getPosition() * 640 / 5 + 640 / 2),
							0,
							(int) (targetTrajectory.getTarget(iteration)
									.getLeftRightPositionVelocity()
									.getPosition() * 640 / 5 + 640 / 2), 480);
			image.getGraphics()
					.drawLine(
							0,
							480 / 2 - (int) (targetTrajectory
									.getTarget(iteration)
									.getUpDownPositionVelocity().getPosition() * 480 / 5),
							640,
							480 / 2 - (int) (targetTrajectory
									.getTarget(iteration)
									.getUpDownPositionVelocity().getPosition() * 480 / 5));
			image.getGraphics()
					.drawRect(
							(int) (targetTrajectory.getTarget(iteration)
									.getLeftRightPositionVelocity()
									.getPosition() * 640 / 5) - 100 + 640 / 2,
							480 / 2 - 100 - (int) (targetTrajectory
									.getTarget(iteration)
									.getUpDownPositionVelocity().getPosition() * 480 / 5),
							200, 200);
		}
